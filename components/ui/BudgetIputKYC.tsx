...
const BudgetInputKYC: React.FC<Props> = ({
    label,
    value,
    onChange,
    currency = 'LAK',
                ...
                <View className="flex-row mb-4 space-x-4 gap-2">
                    {['PER_HOUR', 'PER_DAY', 'PER_JOB' ].map((type) => (
                        <Pressable
                            ...
                            className={`flex-1 border py-4 rounded-xl items-center ${rateType === type ? 'border-primary bg-primary' : 'border-border'}`}
                        >
                            <View className="flex-row items-center">
                                {rateType === type && <Ionicons name="checkmark-circle" size={16} color="#fff" />}
                                <Text className={`${rateType === type ? 'text-white' : 'text-text'} text-caption capitalize ml-1`}>
                                    {type === "PER_JOB" && `${t('kyc.step3.rateType.perJob')}` } 
                                    {type === "PER_HOUR" && `${t('kyc.step3.rateType.perHour')}`}
                                    {type === "PER_DAY" && `${t('kyc.step3.rateType.perDay')}`}
                                </Text>
                            </View>
                        </Pressable>
                    ))}
